...
import org.webrtc.awesome.inter.ISkyEvent;
...
public class VoipEvent implements ISkyEvent {
    ...
    public void shouldStartRing(boolean isComing) {
        if (isComing) {
            Uri uri = Uri.parse("android.resource://" + SkyWebrtcUtils.appContext.getPackageName() + "/" + R.raw.incoming_call_ring);
            ringPlayer.play(SkyWebrtcUtils.appContext, uri, true, AudioManager.STREAM_RING);
        } else {
            Uri uri = Uri.parse("android.resource://" + SkyWebrtcUtils.appContext.getPackageName() + "/" + R.raw.wr_ringback);
            ringPlayer.play(SkyWebrtcUtils.appContext, uri, true, AudioManager.STREAM_RING);
        }
    }
